...
const Catalog = () => {
    ...
    let ProductsElements = products.map( p => <Product name={p.name} price={p.price} color={p.color} country={p.country} conf={p.conf} photo={p.photo}/>);

    let brands = ['Fender', 'Gibson', 'Ibanez', 'Epiphone', 'Jackson', 'ESP', 'B.C.Rich', 'Cort',];

    let BrandsElements = brands.map(b => <div> {b} </div> );
